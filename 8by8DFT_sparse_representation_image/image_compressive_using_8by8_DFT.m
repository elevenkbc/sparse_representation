...
%         temp_dct_thr = temp_dct.*Thr_matrix;
        ...
        temp_dct_thr = temp_dct.*(abs(temp_dct) > 6);
        %IDCT
...
subplot(1,3,1)
imshow(mat2gray(abs(DFT_coe)));
title('DFT 係數')

subplot(1,3,2); imshow(A); colormap('gray')
title('原始圖片')
Ps = psnr(uint8(real(compressive_im)), A);

subplot(1,3,3); imshow(uint8(real(compressive_im))); colormap('gray')
